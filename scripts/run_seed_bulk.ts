...
async function main() {
  ...
  try {
    ...
    let pesananIndex = 1;
    let penagihanIndex = 1;
    
    let pesananValues: string[] = [];
    let penagihanValues: string[] = [];
    let komisiValues: string[] = [];
    ...
    for (const row of data) {
      ...
      for (let i=1; i<=5; i++) {
        ...
        if (angsuran && petugas && parseInt(angsuran) > 0) {
          ...
          const id_penagihan = penagihanIndex++;
          const catatan = `Angsuran ke-${i} via Impor Excel`;
          penagihanValues.push(`(${id_penagihan}, ${id_pesanan}, ${id_penagih}, ${i}, 'BERHASIL', ${nominal}, '${catatan}', NOW())`);
          ...
          komisiValues.push(`('UANG_MASUK', ${komisiSales}, ${id_penagihan}, ${id_sales})`);
          komisiValues.push(`('UANG_MASUK', ${komisiNego}, ${id_penagihan}, ${idNego})`);
          komisiValues.push(`('UANG_MASUK', ${komisiPenagih}, ${id_penagihan}, ${id_penagih})`);
      ...
      pesananValues.push(`(${id_pesanan}, ${id_klien}, ${id_barang}, ${qty}, ${total_harga}, '${status}', ${id_sales}, ${idNego})`);
    }

    console.log(`Memasukkan ${pesananValues.length} pesanan secara massal...`);
    
    // Batch into chunks of 1000 to avoid query size limits just in case
    ...
    for (const chunk of chunkArray(pesananValues, 1000)) {
      await client.query(`INSERT INTO "pesanan" (id_pesanan, id_klien, id_barang, qty, total_harga, status, id_sales, id_nego) VALUES ${chunk.join(', ')}`);
    }

    for (const chunk of chunkArray(penagihanValues, 1000)) {
      await client.query(`INSERT INTO "penagihan" (id_penagihan, id_pesanan, id_penagih, percobaan_ke, status, nominal, catatan, tanggal) VALUES ${chunk.join(', ')}`);
    }

    for (const chunk of chunkArray(komisiValues, 1000)) {
      await client.query(`INSERT INTO "komisiLog" (jenis_komisi, nominal_masuk, id_referensi, id_user) VALUES ${chunk.join(', ')}`);
    }
    
    console.log(`Berhasil mengimpor data massal!`);
